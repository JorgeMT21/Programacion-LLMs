...
def generar_caso_de_uso_evaluar_modelo_tweedie():
    """
    Genera un caso de prueba aleatorio (input y output esperado)
    para la función evaluar_modelo_tweedie(df, columnas, columna_objetivo, test_size).
    """
    ...
    # Generar target positivo y algo sesgado
    coef = np.random.uniform(-0.4, 0.8, size=n_features)
    eta = X @ coef
    y = np.exp(eta) + np.random.uniform(0, 1, size=n_rows)
    ...
    model = TweedieRegressor(power=1.5, alpha=0.0)
...
if __name__ == "__main__":
    entrada, salida_esperada = generar_caso_de_uso_evaluar_modelo_tweedie()
